import { clamp, lerp, clamp01, setupCanvas } from '../lib/utils.js';
...
export function createChapter(theme) {
    ...
    function drawScanlines(cv) {
        ...
        const img = ctx.createImageData(W, H);
        for (let y = 0; y < H; y++) {
            const dim = y % 3 === 0 ? 0.3 : 1;
            for (let x = 0; x < W; x++) {
                const col = lerp(PINK, CORAL, x / W);
                const i = (y * W + x) * 4;
    ...
    function drawChannelShift(cv) {
        ...
        for (let y = 0; y < H; y++) {
            for (let x = 0; x < W; x++) {
                const tR = clamp01(clamp(x - shift, 0, W - 1) / W);
                const tG = x / W;
                const tB = clamp01(clamp(x + shift, 0, W - 1) / W);
    ...
    function drawLowRes(cv) {
        ...
        for (let row = 0; row < rows; row++) {
            for (let col = 0; col < cols; col++) {
                ...
                const c = lerp(PINK, YELLOW, clamp01(t + jitter));
