import { lerp, clamp01, clampPx, hash, setupCanvas } from '../lib/utils.js';
...
export function createChapter(theme) {
    ...
    function setup(id) {
        ...
        const bCols = Math.ceil(W / B);
        const bRows = Math.ceil(H / B);
        for (let br = 0; br < bRows; br++) {
            for (let bc = 0; bc < bCols; bc++) {
                const jitter = (hash(br * 31 + bc * 17) - 0.5) * 0.14;
    ...
    function drawChunk(cv, t) {
        ...
        for (let y = 0; y < H; y++) {
            ...
            const active = hash(band * 11, gf) < 0.35;
            const offset = active ? Math.round((hash(band, gf) - 0.5) * W * 0.75) : 0;
            for (let x = 0; x < W; x++) {
                const srcX = clampPx(x - offset, W);
                const s = (y * W + srcX) * 4;
                const d = (y * W + x) * 4;
                img.data[d] = base[s];
                img.data[d + 1] = base[s + 1];
                img.data[d + 2] = base[s + 2];
                img.data[d + 3] = 255;
            }
        }
    ...
    function drawDissolve(cv, t) {
        ...
        for (let y = 0; y < H; y++) {
            for (let x = 0; x < W; x++) {
                ...
                if (b > thresh) {
                    img.data[i] = base[i];
                    img.data[i + 1] = base[i + 1];
                    img.data[i + 2] = base[i + 2];
                    img.data[i + 3] = 255;
                } else {
                    img.data[i] = DEEP[0];
                    img.data[i + 1] = DEEP[1];
    ...
    function drawWarp(cv, t) {
        ...
        const img = ctx.createImageData(W, H);
        for (let y = 0; y < H; y++) {
            const offset = Math.round(Math.sin(y / H * Math.PI * 3 + t * 0.8) * 30 + Math.sin(y / H * Math.PI * 7 - t * 1.3) * 12);
            for (let x = 0; x < W; x++) {
                const srcX = clampPx(x - offset, W);
                const s = (y * W + srcX) * 4;
                const d = (y * W + x) * 4;
    ...
    function drawSkip(cv, t) {
        ...
        const o1 = hash(1, gf) < 0.45 ? Math.round((hash(11, gf) - 0.5) * W * 0.55) : 0;
        const o2 = hash(2, gf) < 0.45 ? Math.round((hash(22, gf) - 0.5) * W * 0.45) : 0;
        for (let y = 0; y < H; y++) {
    ...
    function drawMirror(cv, t) {
        ...
        for (let y = 0; y < H; y++) {
            for (let bi = 0; bi < nB; bi++) {
                ...
                const flipped = hash(bi * 7, gf) < 0.4;
                for (let x = x0; x < x1; x++) {
                    const srcX = flipped ? (x0 + x1 - 1 - x) : x;
                    const s = (y * W + clampPx(srcX, W)) * 4;
                    const d = (y * W + x) * 4;
                    img.data[d] = base[s];
                    img.data[d + 1] = base[s + 1];
                    img.data[d + 2] = base[s + 2];
    ...
    function drawScatter(cv, t) {
        ...
        for (let y = 0; y < H; y++) {
            for (let x = 0; x < W; x++) {
                const i = (y * W + x) * 4;
                if (hash(x * 3 + y * 7, gf) < density) {
                    const dx = Math.round((hash(x + y * 13, gf) - 0.5) * 42);
                    const dy = Math.round((hash(x * 2 + y * 5, gf) - 0.5) * 10);
                    const srcX = clampPx(x + dx, W);
                    const srcY = Math.max(0, Math.min(H - 1, y + dy));
                    const s = (srcY * W + srcX) * 4;
                    img.data[i] = base[s];
                    img.data[i + 1] = base[s + 1];
                    img.data[i + 2] = base[s + 2];
                    img.data[i + 3] = 255;
                } else {
                    img.data[i] = base[i];
                    img.data[i + 1] = base[i + 1];
    ...
    function drawTile(cv, t) {
        ...
        for (let br = 0; br < bRows; br++) {
            for (let bc = 0; bc < bCols; bc++) {
                const corrupt = hash(br * 23 + bc * 17, gf) < 0.14;
                const shift = corrupt ? Math.round((hash(br + bc * 2, gf) - 0.5) * 5) : 0;
                const srcBc = Math.max(0, Math.min(bCols - 1, bc + shift));
                const jitter = (hash(br * 7 + bc * 13) - 0.5) * 0.06;
